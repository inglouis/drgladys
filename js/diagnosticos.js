...
//GENERA LOS COMPORTAMIENTOS BÁSICOS DE LOS POPUPS
/////////////////////////////////////////////////////
window.contenedores = new ContenedoresEspeciales('diagnosticos-filtros')
...
/////////////////////////////////////////////////////
window.cargar = new paginaCargada('#tabla-diagnosticos thead .ASC', 'existencia')
...
//										diagnosticos                                           
//----------------------------------------------------------------------------------------------------
class Diagnosticos extends Acciones {

	constructor(crud) {
		super(crud)
		this.fila
		this.uso   = 'diagnosticos'
		this.funcion = 'buscar_diagnosticos'
		this.cargar  = 'cargar_diagnosticos' 
		//-------------------------------
		this.alternar = [true, 'white', 'whitesmoke']
		this.especificos = ['id_diagnostico', 'nombre']
		this.limitante = 0
		this.boton = ''
		//-------------------------------
...
/////////////////////////////////////////////////////
///
diagnosticos['crud']['customBodyEvents'] = {
	"informacion": async (e) => {
...
(async () => {
	var resultado = await tools.fullAsyncQuery('diagnosticos', 'cargar_diagnosticos', [])
	diagnosticos.cargarTabla(JSON.parse(resultado), undefined, undefined)
})()
...
qs('#procesar').addEventListener('click', async e => {
	...
	diagnosticos.spinner('#tabla-diagnosticos tbody')
	
	var peticion = contenedores.procesar(tools.fullQuery, 'diagnosticos','filtrar')

	peticion.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
        	qs('#tabla-diagnosticos tbody').innerHTML = ''
        	diagnosticos.cargarTabla(JSON.parse(this.responseText))
        }
    };
})
	...
	if(e.target.classList.contains('editar')) {
		...
		if(window.procesar) {
			...
			if (datos !== '') {
				...
				var resultado = await tools.fullAsyncQuery('diagnosticos', 'actualizar_diagnosticos', datos)

				if (resultado.trim() === 'exito') {

					diagnosticos.confirmarActualizacion(ediPop)
...
qs('#crud-insertar-botones').addEventListener('click', async e => {
	...
	if (e.target.classList.contains('insertar')) {
		...
		if (datos !== '') {
			...
			var resultado = await tools.fullAsyncQuery('diagnosticos', 'crear_diagnosticos', datos)

			if (resultado.trim() === 'exito') {

				diagnosticos.confirmarActualizacion(insPop)
...
/* -------------------------------------------------------------------------------------------------*/
qs('#tabla-diagnosticos').addEventListener('click', e => {
...
qs('#diagnosticos-insertar').addEventListener('click', e => {
