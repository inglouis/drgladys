...
//GENERA LOS COMPORTAMIENTOS BÁSICOS DE LOS POPUPS
/////////////////////////////////////////////////////
window.contenedores = new ContenedoresEspeciales('estado_civil-filtros')
...
/////////////////////////////////////////////////////
window.cargar = new paginaCargada('#tabla-estado_civil thead .ASC', 'existencia')
...
//										estado_civil                                           
//----------------------------------------------------------------------------------------------------
class Estado_civil extends Acciones {

	constructor(crud) {
		super(crud)
		this.fila
		this.uso   = 'estado_civil'
		this.funcion = 'buscar_estado_civil'
		this.cargar  = 'cargar_estado_civil' 
		//-------------------------------
		this.alternar = [true, 'white', 'whitesmoke']
		this.especificos = ['id_estado_civil', 'nombre']
		this.limitante = 0
		this.boton = ''
		//-------------------------------
...
/////////////////////////////////////////////////////
///
estado_civil['crud']['customBodyEvents'] = {
	"informacion": async (e) => {
...
(async () => {
	var resultado = await tools.fullAsyncQuery('estado_civil', 'cargar_estado_civil', [])
	estado_civil.cargarTabla(JSON.parse(resultado), undefined, undefined)
})()
...
qs('#procesar').addEventListener('click', async e => {
	...
	estado_civil.spinner('#tabla-estado_civil tbody')
	
	var peticion = contenedores.procesar(tools.fullQuery, 'estado_civil','filtrar')

	peticion.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
        	qs('#tabla-estado_civil tbody').innerHTML = ''
        	estado_civil.cargarTabla(JSON.parse(this.responseText))
        }
    };
})
	...
	if(e.target.classList.contains('editar')) {
		...
		if(window.procesar) {
			...
			if(datos !== '') {
				...
				var resultado = await tools.fullAsyncQuery('estado_civil', 'actualizar_estado_civil', datos)

				if (resultado.trim() === 'exito') {

					estado_civil.confirmarActualizacion(ediPop)
...
qs('#crud-insertar-botones').addEventListener('click', async e => {
	...
	if (e.target.classList.contains('insertar')) {
		...
		if (datos !== false) {
			...
			var resultado = await tools.fullAsyncQuery('estado_civil', 'crear_estado_civil', datos)

			if (resultado.trim() === 'exito') {

				estado_civil.confirmarActualizacion(insPop)
...
/* -------------------------------------------------------------------------------------------------*/
qs('#tabla-estado_civil').addEventListener('click', e => {
...
qs('#estado_civil-insertar').addEventListener('click', e => {
