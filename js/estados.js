...
//GENERA LOS COMPORTAMIENTOS BÁSICOS DE LOS POPUPS
/////////////////////////////////////////////////////
window.contenedores = new ContenedoresEspeciales('estados-filtros')
...
/////////////////////////////////////////////////////
window.cargar = new paginaCargada('#tabla-estados thead .ASC', 'existencia')
...
//										estados                                           
//----------------------------------------------------------------------------------------------------
class Estados extends Acciones {

	constructor(crud) {
		super(crud)
		this.fila
		this.uso   = 'estados'
		this.funcion = 'buscar_estados'
		this.cargar  = 'cargar_estados' 
		//-------------------------------
		this.alternar = [true, 'white', 'whitesmoke']
		this.especificos = ['id_estado', 'nombre']
		this.limitante = 0
		this.boton = ''
		//-------------------------------
...
(async () => {
	var resultado = await tools.fullAsyncQuery('estados', 'cargar_estados', [])
	estados.cargarTabla(JSON.parse(resultado), undefined, undefined)
})()
...
qs('#procesar').addEventListener('click', async e => {
	...
	estados.spinner('#tabla-estados tbody')
	
	var peticion = contenedores.procesar(tools.fullQuery, 'estados','filtrar')

	peticion.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
        	qs('#tabla-estados tbody').innerHTML = ''
        	estados.cargarTabla(JSON.parse(this.responseText))
        }
    };
})
	...
	if(e.target.classList.contains('editar')) {
		...
		if(window.procesar) {
			...
			if(datos !== '') {
				...
				var resultado = await tools.fullAsyncQuery('estados', 'actualizar_estados', datos)

				if (resultado.trim() === 'exito') {

					estados.confirmarActualizacion(ediPop)
...
qs('#crud-insertar-botones').addEventListener('click', async e => {
	...
	if (e.target.classList.contains('insertar')) {
		...
		if (datos !== false) {
			...
			var resultado = await tools.fullAsyncQuery('estados', 'crear_estados', datos)

			if (resultado.trim() === 'exito') {

				estados.confirmarActualizacion(insPop)
...
/* -------------------------------------------------------------------------------------------------*/
qs('#tabla-estados').addEventListener('click', e => {
...
qs('#estados-insertar').addEventListener('click', e => {
