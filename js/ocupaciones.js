...
//GENERA LOS COMPORTAMIENTOS BÁSICOS DE LOS POPUPS
/////////////////////////////////////////////////////
window.contenedores = new ContenedoresEspeciales('ocupaciones-filtros')
...
/////////////////////////////////////////////////////
window.cargar = new paginaCargada('#tabla-ocupaciones thead .ASC', 'existencia')
...
//										ocupaciones                                           
//----------------------------------------------------------------------------------------------------
class Ocupaciones extends Acciones {

	constructor(crud) {
		super(crud)
		this.fila
		this.uso   = 'ocupaciones'
		this.funcion = 'buscar_ocupaciones'
		this.cargar  = 'cargar_ocupaciones' 
		//-------------------------------
		this.alternar = [true, 'white', 'whitesmoke']
		this.especificos = ['id_ocupacion', 'nombre']
		this.limitante = 0
		this.boton = ''
		//-------------------------------
...
/////////////////////////////////////////////////////
///
ocupaciones['crud']['customBodyEvents'] = {
	"informacion": async (e) => {
...
(async () => {
	var resultado = await tools.fullAsyncQuery('ocupaciones', 'cargar_ocupaciones', [])
	ocupaciones.cargarTabla(JSON.parse(resultado), undefined, undefined)
})()
...
qs('#procesar').addEventListener('click', async e => {
	...
	ocupaciones.spinner('#tabla-ocupaciones tbody')
	
	var peticion = contenedores.procesar(tools.fullQuery, 'ocupaciones','filtrar')

	peticion.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
        	qs('#tabla-ocupaciones tbody').innerHTML = ''
        	ocupaciones.cargarTabla(JSON.parse(this.responseText))
        }
    };
})
	...
	if(e.target.classList.contains('editar')) {
		...
		if(window.procesar) {
			...
			if(datos !== '') {
				...
				var resultado = await tools.fullAsyncQuery('ocupaciones', 'actualizar_ocupaciones', datos)

				if (resultado.trim() === 'exito') {

					ocupaciones.confirmarActualizacion(ediPop)
...
qs('#crud-insertar-botones').addEventListener('click', async e => {
	...
	if (e.target.classList.contains('insertar')) {
		...
		if (datos !== false) {
			...
			var resultado = await tools.fullAsyncQuery('ocupaciones', 'crear_ocupaciones', datos)

			if (resultado.trim() === 'exito') {

				ocupaciones.confirmarActualizacion(insPop)
...
/* -------------------------------------------------------------------------------------------------*/
qs('#tabla-ocupaciones').addEventListener('click', e => {
...
qs('#ocupaciones-insertar').addEventListener('click', e => {
