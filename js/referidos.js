...
//GENERA LOS COMPORTAMIENTOS BÁSICOS DE LOS POPUPS
/////////////////////////////////////////////////////
window.contenedores = new ContenedoresEspeciales('referidos-status')
...
/////////////////////////////////////////////////////
window.cargar = new paginaCargada('#tabla-referidos thead .ASC', 'existencia')
...
//										referidos                                           
//----------------------------------------------------------------------------------------------------
class Referidos extends Acciones {

	constructor(crud) {
		super(crud)
		this.fila
		this.uso   = 'referidos'
		this.funcion = 'buscar_referidos'
		this.cargar  = 'cargar_referidos' 
		//-------------------------------
		this.alternar = [true, 'white', 'whitesmoke']
		this.especificos = ['id_referido', 'nombre']
		this.limitante = 0
		this.boton = ''
		//-------------------------------
...
/////////////////////////////////////////////////////
///
window.referidos = new Referidos(new Tabla(
	[
		['Nombre del médico referido', true, 1],
		['Status', true, 2],
		['Acciones', false, 0]
	],
	'tabla-referidos', 'busqueda', Number(sesiones.modo_filas), 'izquierda', 'derecha', 'numeracion', true
))
/////////////////////////////////////////////////////
///
referidos['crud'].generarColumnas(['gSpan', null, null], [false],['HTML'], '', 1)
referidos['crud'].generarColumnas(['gSpan', null, null], [false],['HTML'], '', 2)

referidos['crud'].cuerpo.push([referidos['crud'].columna = referidos['crud'].cuerpo.length, [
	referidos['crud'].gBt(['editar btn btn-editar', '', 'padding: 3px;'], `<svg class="iconos-b" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M497.9 142.1l-46.1 46.1c-4.7 4.7-12.3 4.7-17 0l-111-111c-4.7-4.7-4.7-12.3 0-17l46.1-46.1c18.7-18.7 49.1-18.7 67.9 0l60.1 60.1c18.8 18.7 18.8 49.1 0 67.9zM284.2 99.8L21.6 362.4.4 483.9c-2.9 16.4 11.4 30.6 27.8 27.8l121.5-21.3 262.6-262.6c4.7-4.7 4.7-12.3 0-17l-111-111c-4.8-4.7-12.4-4.7-17.1 0zM124.1 339.9c-5.5-5.5-5.5-14.3 0-19.8l154-154c5.5-5.5 14.3-5.5 19.8 0s5.5 14.3 0 19.8l-154 154c-5.5 5.5-14.3 5.5-19.8 0zM88 424h48v36.3l-64.5 11.3-31.1-31.1L51.7 376H88v48z"></path></svg>`),
], [false], ['VALUE'], '', 0])

/////////////////////////////////////////////////////
referidos['crud']['limitante'] = 1

/////////////////////////////////////////////////////
///
referidos['crud'].botonModoBusqueda("#modo-buscar", 1, [
	['id_referido'],
	['id_referido', 'nombre']
], {"mensaje": (e) => {
...
/////////////////////////////////////////////////////
///
referidos['crud']['customBodyEvents'] = {
	/* -------------------------------------------------------------------------------------------------*/
...
(async () => {
	var resultado = await tools.fullAsyncQuery('referidos', 'cargar_referidos', [])
	referidos.cargarTabla(JSON.parse(resultado), undefined, undefined)
})()
...
qs('#procesar').addEventListener('click', async e => {
	...
	referidos.spinner('#tabla-referidos tbody')
	
	var peticion = contenedores.procesar(tools.fullQuery, 'referidos','filtrar')

	peticion.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
        	qs('#tabla-referidos tbody').innerHTML = ''
        	referidos.cargarTabla(JSON.parse(this.responseText))
        }
    };
})
	...
	if(e.target.classList.contains('editar')) {
		...
		if(window.procesar) {
			...
			if (datos !== '') {
				...
				var resultado = await tools.fullAsyncQuery('referidos', 'actualizar_referidos', datos)

				if (resultado.trim() === 'exito') {

					referidos.confirmarActualizacion(ediPop)
...
qs('#crud-insertar-botones').addEventListener('click', async e => {
	...
	if (e.target.classList.contains('insertar')) {
		...
		if (datos !== '') {
			...
			var resultado = await tools.fullAsyncQuery('referidos', 'crear_referidos', datos)

			if (resultado.trim() === 'exito') {

				referidos.confirmarActualizacion(insPop)
...
/* -------------------------------------------------------------------------------------------------*/
qs('#tabla-referidos').addEventListener('click', e => {
...
qs('#referidos-insertar').addEventListener('click', e => {
