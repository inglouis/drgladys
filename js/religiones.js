...
//GENERA LOS COMPORTAMIENTOS BÁSICOS DE LOS POPUPS
/////////////////////////////////////////////////////
window.contenedores = new ContenedoresEspeciales('religiones-filtros')
...
/////////////////////////////////////////////////////
window.cargar = new paginaCargada('#tabla-religiones thead .ASC', 'existencia')
...
//										religiones                                           
//----------------------------------------------------------------------------------------------------
class Religiones extends Acciones {

	constructor(crud) {
		super(crud)
		this.fila
		this.uso   = 'religiones'
		this.funcion = 'buscar_religiones'
		this.cargar  = 'cargar_religiones' 
		//-------------------------------
		this.alternar = [true, 'white', 'whitesmoke']
		this.especificos = ['id_religion', 'nombre']
		this.limitante = 0
		this.boton = ''
		//-------------------------------
...
/////////////////////////////////////////////////////
///
religiones['crud']['customBodyEvents'] = {
	"informacion": async (e) => {
...
(async () => {
	var resultado = await tools.fullAsyncQuery('religiones', 'cargar_religiones', [])
	religiones.cargarTabla(JSON.parse(resultado), undefined, undefined)
})()
...
qs('#procesar').addEventListener('click', async e => {
	...
	religiones.spinner('#tabla-religiones tbody')
	
	var peticion = contenedores.procesar(tools.fullQuery, 'religiones','filtrar')

	peticion.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
        	qs('#tabla-religiones tbody').innerHTML = ''
        	religiones.cargarTabla(JSON.parse(this.responseText))
        }
    };
})
	...
	if(e.target.classList.contains('editar')) {
		...
		if(window.procesar) {
			...
			if(datos !== '') {
				...
				var resultado = await tools.fullAsyncQuery('religiones', 'actualizar_religiones', datos)

				if (resultado.trim() === 'exito') {

					religiones.confirmarActualizacion(ediPop)
...
qs('#crud-insertar-botones').addEventListener('click', async e => {
	...
	if (e.target.classList.contains('insertar')) {
		...
		if (datos !== false) {
			...
			var resultado = await tools.fullAsyncQuery('religiones', 'crear_religiones', datos)

			if (resultado.trim() === 'exito') {

				religiones.confirmarActualizacion(insPop)
...
/* -------------------------------------------------------------------------------------------------*/
qs('#tabla-religiones').addEventListener('click', e => {
...
qs('#religiones-insertar').addEventListener('click', e => {
